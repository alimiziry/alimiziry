...
export const Dashboard: React.FC = () => {
  ...
  const handleResetVisits = async () => {
    if (confirm("هل أنت متأكد من تصفير عداد الزيارات الأسبوعي لجميع العملاء؟\nسيتم تحويل جميع الحالات إلى 'لم تتم'.")) {
      try {
        setIsResetting(true);
        await dataService.resetWeeklyVisits();
        alert("تم تصفير الزيارات بنجاح.");
        window.location.reload(); // Force reload to ensure data reflects everywhere
      } catch (error) {
